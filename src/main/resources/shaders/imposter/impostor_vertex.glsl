...
void main() {
  const float GLOW_RADIUS_FACTOR = 8;
  Body b = srcB.bodies[gl_InstanceID];
  vec3 center = b.posMass.xyz;
  float mass = b.posMass.w;
  trueRadius = cbrt(max(mass, 0.0)) * uPointScale;
  ...
  if (vid == 0) vMapping = vec2(-1.0, -1.0);
  else if (vid == 1) vMapping = vec2(-1.0,  1.0);
  else if (vid == 2) vMapping = vec2( 1.0, -1.0);
  else               vMapping = vec2( 1.0,  1.0);
