...
void main() {
	...
	vec3 minC = relativeLocation(vec3(node.aabb[0], node.aabb[1], node.aabb[2]), uRelativeTo)*units.len;
	vec3 maxC = relativeLocation(vec3(node.aabb[3], node.aabb[4], node.aabb[5]), uRelativeTo)*units.len;
