...
public class OldGPUSimulation {
    ...
    private ComputeShader COMPUTE_AABB_KERNEL;
    private ComputeShader COLLAPSE_AABB_KERNEL;
    private ComputeShader MORTON_KERNEL;
    private ComputeShader RADIX_SORT_HISTOGRAM_KERNEL;
    private ComputeShader RADIX_SORT_PARALLEL_SCAN_KERNEL;
    private ComputeShader RADIX_SORT_EXCLUSIVE_SCAN_KERNEL;
    private ComputeShader RADIX_SORT_SCATTER_KERNEL;
    private ComputeShader BUILD_BINARY_RADIX_TREE_KERNEL;
    private ComputeShader INIT_LEAVES_KERNEL;
    private ComputeShader PROPAGATE_NODES_KERNEL;
    private ComputeShader COMPUTE_FORCE_KERNEL;
    private ComputeShader DEBUG_KERNEL;
    ...
    public OldGPUSimulation(ArrayList<Planet> planets) {
    ...
    private void createRenderShaders() {
    ...
    private void createComputeShaders() {
        
        //Compute AABB Kernel
        COMPUTE_AABB_KERNEL = new ComputeShader("KERNEL_COMPUTE_AABB", this);
        COMPUTE_AABB_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
            numWorkGroupsUniform
        });
        COMPUTE_AABB_KERNEL.setSSBOs(new String[] {
            "AABB_SSBO",
            "SWAPPING_BODIES_IN_SSBO"
        });
        COMPUTE_AABB_KERNEL.setXWorkGroupsFunction(() -> {
            return numGroups();
        });

        COLLAPSE_AABB_KERNEL = new ComputeShader("KERNEL_COLLAPSE_AABB", this);
        COLLAPSE_AABB_KERNEL.setUniforms(new Uniform[] {
            numWorkGroupsUniform
        });
        COLLAPSE_AABB_KERNEL.setSSBOs(new String[] {
            "AABB_SSBO",
        });
        COLLAPSE_AABB_KERNEL.setXWorkGroupsFunction(() -> {
            return 1;
        });

        MORTON_KERNEL = new ComputeShader("KERNEL_MORTON", this);
        MORTON_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
        });
        MORTON_KERNEL.setSSBOs(new String[] {
            "MORTON_IN_SSBO",
            "SWAPPING_BODIES_IN_SSBO",
            "INDEX_IN_SSBO",
            "AABB_SSBO"
        ...
        MORTON_KERNEL.setXWorkGroupsFunction(() -> {
            return numGroups();
        });

        RADIX_SORT_HISTOGRAM_KERNEL = new ComputeShader("KERNEL_RADIX_HIST", this);
        RADIX_SORT_HISTOGRAM_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
            passShiftUniform,
            numWorkGroupsUniform
        });
        RADIX_SORT_HISTOGRAM_KERNEL.setSSBOs(new String[] {
            "CURRENT_MORTON_IN_SSBO",
            "CURRENT_INDEX_IN_SSBO",
            "WG_HIST_SSBO",
        });
        RADIX_SORT_HISTOGRAM_KERNEL.setXWorkGroupsFunction(() -> {
            return numGroups();
        });

        RADIX_SORT_PARALLEL_SCAN_KERNEL = new ComputeShader("KERNEL_RADIX_PARALLEL_SCAN", this);
        RADIX_SORT_PARALLEL_SCAN_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
            numWorkGroupsUniform
        });
        RADIX_SORT_PARALLEL_SCAN_KERNEL.setSSBOs(new String[] {
            "WG_HIST_SSBO",
            "WG_SCANNED_SSBO",
            "GLOBAL_BASE_SSBO",
        });
        RADIX_SORT_PARALLEL_SCAN_KERNEL.setXWorkGroupsFunction(() -> {
            return NUM_RADIX_BUCKETS;
        });

        RADIX_SORT_EXCLUSIVE_SCAN_KERNEL = new ComputeShader("KERNEL_RADIX_EXCLUSIVE_SCAN", this);
        RADIX_SORT_EXCLUSIVE_SCAN_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
            numWorkGroupsUniform
        });
        RADIX_SORT_EXCLUSIVE_SCAN_KERNEL.setSSBOs(new String[] {
            "GLOBAL_BASE_SSBO",
            "BUCKET_TOTALS_SSBO",
        });
        RADIX_SORT_EXCLUSIVE_SCAN_KERNEL.setXWorkGroupsFunction(() -> {
            return 1;
        });

        RADIX_SORT_SCATTER_KERNEL = new ComputeShader("KERNEL_RADIX_SCATTER", this);

        RADIX_SORT_SCATTER_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
            passShiftUniform,
            numWorkGroupsUniform
        });

        RADIX_SORT_SCATTER_KERNEL.setSSBOs(new String[] {
            "CURRENT_MORTON_IN_SSBO",
            "CURRENT_INDEX_IN_SSBO",
            "WG_SCANNED_SSBO",
            "GLOBAL_BASE_SSBO",
        ...
        RADIX_SORT_SCATTER_KERNEL.setXWorkGroupsFunction(() -> {
            return numGroups();
        });


        BUILD_BINARY_RADIX_TREE_KERNEL = new ComputeShader("KERNEL_BUILD_BINARY_RADIX_TREE", this);
        BUILD_BINARY_RADIX_TREE_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
        });
        
        BUILD_BINARY_RADIX_TREE_KERNEL.setSSBOs(new String[] {
            "CURRENT_MORTON_IN_SSBO",
            "CURRENT_INDEX_IN_SSBO",
            "NODES_SSBO",
        });

        BUILD_BINARY_RADIX_TREE_KERNEL.setXWorkGroupsFunction(() -> {
        ...
        INIT_LEAVES_KERNEL = new ComputeShader("KERNEL_INIT_LEAVES", this);

        INIT_LEAVES_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
        }); 

        INIT_LEAVES_KERNEL.setSSBOs(new String[] {
            "SWAPPING_BODIES_IN_SSBO",
            "NODES_SSBO",
            "MORTON_IN_SSBO",
            "INDEX_IN_SSBO",
        ...
        INIT_LEAVES_KERNEL.setXWorkGroupsFunction(() -> {
            return numGroups();
        });

        PROPAGATE_NODES_KERNEL = new ComputeShader("KERNEL_PROPAGATE_NODES", this);

        PROPAGATE_NODES_KERNEL.setUniforms(new Uniform[] {
        });

        PROPAGATE_NODES_KERNEL.setSSBOs(new String[] {
            "NODES_SSBO",
            "WORK_QUEUE_SSBO"
        });

        PROPAGATE_NODES_KERNEL.setXWorkGroupsFunction(() -> {
            int maxPossibleNodes = planets.size() - 1; // Internal nodes
            int workGroups = (maxPossibleNodes + WORK_GROUP_SIZE - 1) / WORK_GROUP_SIZE;
            return workGroups;
        });

        COMPUTE_FORCE_KERNEL = new ComputeShader("KERNEL_COMPUTE_FORCE", this);

        COMPUTE_FORCE_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
            thetaUniform,
            dtUniform,
            elasticityUniform,
        ...
        COMPUTE_FORCE_KERNEL.setSSBOs(new String[] {
            "SWAPPING_BODIES_OUT_SSBO",
            "NODES_SSBO",
        });

        COMPUTE_FORCE_KERNEL.setXWorkGroupsFunction(() -> {
            return numGroups();
        });

        DEBUG_KERNEL = new ComputeShader("KERNEL_DEBUG", this);

        DEBUG_KERNEL.setUniforms(new Uniform[] {
            numBodiesUniform,
        });

        DEBUG_KERNEL.setSSBOs(new String[] {
            "CURRENT_MORTON_IN_SSBO",
            "CURRENT_INDEX_IN_SSBO",
        });

        DEBUG_KERNEL.setXWorkGroupsFunction(() -> {
            return numGroups();
        });
    ...
    private void createSSBOs() {
        ...
        // These are pointers to fixed SSBOs, so dont need to create them
    ...
    public void cleanupEmbedded() {
        ...
        if (COMPUTE_AABB_KERNEL != null) COMPUTE_AABB_KERNEL.delete();
        if (COLLAPSE_AABB_KERNEL != null) COLLAPSE_AABB_KERNEL.delete();
        if (MORTON_KERNEL != null) MORTON_KERNEL.delete();
    ...
    private void radixSort() {
        ...
        for (int pass = 0; pass < numPasses; pass++) {
            ...
            // Phase 1: Histogram
            RADIX_SORT_HISTOGRAM_KERNEL.run();
            ...
            RADIX_SORT_PARALLEL_SCAN_KERNEL.run();

            //System.out.println("Radix sort parallel scan");
           // debugRadixSort();

            RADIX_SORT_EXCLUSIVE_SCAN_KERNEL.run();
    ...
    private void buildBinaryRadixTree() {
        ...
        BUILD_BINARY_RADIX_TREE_KERNEL.run();
    ...
    private void computeCOMAndLocation() {
        ...
        INIT_LEAVES_KERNEL.run();

        //debugTree();


        for (int i = 0; i < PROPAGATE_NODES_ITERATIONS; i++) {
            PROPAGATE_NODES_KERNEL.run();
        }
    }

        private void computeForce() {
        COMPUTE_FORCE_KERNEL.run();
